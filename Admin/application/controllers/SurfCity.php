...
class SurfCity extends CI_Controller {
  function __construct() {
	...
	public function loginOrSignUpCheck(){
		$success = false;
    		...
    		$this->load->model('Surf_City');
    		$isNumber = $this->Surf_City->loginOrSignUpCheck($user_id);
    		$id = '';
    		if ($isNumber) {    			
  ...
  public function registerUser()
  {
    ...
    $this->load->model('Surf_City');
    $result=$this->Surf_City->registerUser($first_name,$password,$address,$mobile_no,$last_name);
    if($result==202)
      {
        $success = false;
  ...
  public function checkLogin()
  {
    ...
    $this->load->model('Surf_City');
    $userId = array('id' => 0 );
	if ($userId = $this->Surf_City->checkLogin($user_id,$password)) {
      		$success = true;
	...
	public function getCity(){
		$success = false;
    		$message = 'Data Fetch Error';
    		$this->load->model('Surf_City');
			if ($city = $this->Surf_City->getCity()) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getShopListByCity(){
		$success = false;
    		...
    		if($this->input->post('shop_city')){
    			$this->load->model('Surf_City');
			if ($foodList = $this->Surf_City->getShopListByCity($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getFoodListByCity(){
		$success = false;
    		...
    		if($this->input->post('shop_city')&&$this->input->post('owner_id')){
    			$this->load->model('Surf_City');
			if ($foodList = $this->Surf_City->getFoodListByCity($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getOfferFoodListByCity(){
		$success = false;
    		...
    		if($this->input->post('shop_city')){
    			$this->load->model('Surf_City');
			if ($foodList = $this->Surf_City->getOfferFoodListByCity($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getShopImages(){
		$success = false;
    		...
    		if($this->input->post('owner_id')){
    			$this->load->model('Surf_City');
			if ($foodList = $this->Surf_City->getShopImages($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getCartList(){
		$success = false;
    		...
    		if($this->input->post('user_id')){
    			$this->load->model('Surf_City');
			if ($foodList = $this->Surf_City->getCartList($this->input->post())) {
				if($foodList==202){
					$success = false;
	...
	public function getUserDetails(){
		$success = false;
    		...
    		if($this->input->post('user_id')){
    			$this->load->model('Surf_City');
			if ($foodList = $this->Surf_City->getUserDetails($this->input->post())) {
				$success = true;
      				$message = 'Ok';      				
			}else{
      				$success = false;
	...
	public function setPaymentDetails(){
		$success = false;
    		...
    		if($this->input->post('user_id')){
    			$this->load->model('Surf_City');
			if ($foodList = $this->Surf_City->setPaymentDetails($this->input->post())) {
				$success = true;
      				$message = 'Ok';      				
			}else{
      				$success = false;
	...
	public function getCatAndSubCatList(){
		$success = false;
    		...
    		$foodList = array();
    		$this->load->model('Surf_City');
		if ($dataList = $this->Surf_City->getCatAndSubCatList()) {
      			$success = true;
      			$message = 'Ok';
		}else{
      			$success = false;
		...
		$ar = array('user_id'=>$this->input->post('user_id'));
    		if($dataList = $this->Surf_City->getUserInCart($ar)){
    			...
    			if($dataList = $this->Surf_City->getInCartShopCheck($array)){
    				...
    				if($dataList = $this->Surf_City->getItemCheck($arrayi)){
    					//have same shop in cart---OK
    					$success = true;
		...
		$success = false;
    		...
    		if($dataList = $this->Surf_City->getUserInCart($ar)){
    			...
    			if($dataList = $this->Surf_City->getInCartShopCheck($array)){
    				...
    				if($idItem = $this->Surf_City->updateItemToCart($cartItemArray,$idItemCart)){
    					$arrayQut = array('id'=>$idItem);
    					$quantity = $this->Surf_City->getItemCheck($arrayQut);
    					$id = '';
    				...
    				if($idItem = $this->Surf_City->addItemToCart($cartItemArray)){
    					$arrayQut = array('id'=>$idItem);
    					$quantity = $this->Surf_City->getItemCheck($arrayQut);
    					$id = '';
    					foreach ($quantity as $key => $value) {
    			...
    			if($id = $this->Surf_City->addToCart($cartArray)){
		...
		$success = false;
    		...
    		if($this->input->post('item_id')&&$this->input->post('owner_id')&&$this->input->post('user_id')){
    			...
    			$qut = $this->input->post('quantity');
    				...
    				if($qut>0){
    				$cartItemArray = array('quantity'=>$this->input->post('quantity'),'price'=>$this->input->post('price'));
    				if($idItem = $this->Surf_City->updateRmItemToCart($cartItemArray,$idItemCart)){
    					$arrayQut = array('id'=>$idItem);
    					$quantity = $this->Surf_City->getItemCheck($idItemCart);
    					$id = '';
    					foreach ($quantity as $key => $value) {
    				  		$id = $value->quantity;
    					}
    					...
    					$this->Surf_City->removeItemFromCart($idItemCart,$recart);
    					$success = true;
    					$message = 'Quantity Removed From Cart';
    					$msgCart = 0;
    				}
   ...
   public function getBookingHistory()
    {
      ...
      $this->load->model('Surf_City');
      $checkProfile = $this->Surf_City->getBookingHistory($this->input->post());
     ...
     public function changeForgotPass()
    {
      ...
      $this->load->model('Surf_City');
      $checkProfile = $this->Surf_City->changeForgotPass($phone,$user_password);
      if ($checkProfile) {
        $success = true;
  ...
  public function bookAppointment()
    ...
    public function getProfileData()
    {
      ...
      if($this->input->post('user_id')){
      $this->load->model('Surf_City');
      $checkProfile = $this->Surf_City->getProfileData($this->input->post('user_id'));
    ...
    public function updateUserInfo()
    {
      ...
      if($this->input->post('user_id')){
      $this->load->model('Surf_City');
      $checkProfile = $this->Surf_City->updateUserInfo($this->input->post());
