...
class SurfCity2 extends CI_Controller {
  function __construct() {
	...
	public function loginOrSignUpCheck(){
		$success = false;
    		...
    		$this->load->model('Surf_City2');
    		$isNumber = $this->Surf_City2->loginOrSignUpCheck($user_id);
    		$id = '';
    		if ($isNumber) {
  ...
  public function registerUser()
  {
    ...
    $this->load->model('Surf_City2');
    $result=$this->Surf_City2->registerUser($first_name,$password,$address,$mobile_no,$last_name);
    if($result==202)
      {
        $success = false;
  ...
  public function checkLogin()
  {
    ...
    $this->load->model('Surf_City2');
    $userId = array('id' => 0 );
	if ($userId = $this->Surf_City2->checkLogin($user_id,$password)) {
      		$success = true;
	...
	public function getCity(){
		$success = false;
    		$message = 'Data Fetch Error';
    		$this->load->model('Surf_City2');
			if ($city = $this->Surf_City2->getCity()) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getShopListByCity(){
		$success = false;
    		...
    		if($this->input->post('shop_city')){
    			$this->load->model('Surf_City2');
			if ($foodList = $this->Surf_City2->getShopListByCity($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getFoodListByCity(){
		$success = false;
    		...
    		if($this->input->post('shop_city')&&$this->input->post('owner_id')){
    			$this->load->model('Surf_City2');
			if ($foodList = $this->Surf_City2->getFoodListByCity($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getOfferFoodListByCity(){
		$success = false;
    		...
    		if($this->input->post('shop_city')){
    			$this->load->model('Surf_City2');
			if ($foodList = $this->Surf_City2->getOfferFoodListByCity($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getShopImages(){
		$success = false;
    		...
    		if($this->input->post('owner_id')){
    			$this->load->model('Surf_City2');
			if ($foodList = $this->Surf_City2->getShopImages($this->input->post())) {
      				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getCartList(){
		$success = false;
    		...
    		if($this->input->post('user_id')){
    			$this->load->model('Surf_City2');
			if ($foodList = $this->Surf_City2->getCartList($this->input->post())) {
				if($foodList==202){
					$success = false;
	...
	public function getUserDetails(){
		$success = false;
    		...
    		if($this->input->post('user_id')){
    			$this->load->model('Surf_City2');
			if ($foodList = $this->Surf_City2->getUserDetails($this->input->post())) {
				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function setPaymentDetails(){
		$success = false;
    		...
    		if($this->input->post('user_id')){
    			$this->load->model('Surf_City2');
			if ($foodList = $this->Surf_City2->setPaymentDetails($this->input->post())) {
				$success = true;
      				$message = 'Ok';
			}else{
      				$success = false;
	...
	public function getCatAndSubCatList(){
		$success = false;
    		...
    		$foodList = array();
    		$this->load->model('Surf_City2');
		if ($dataList = $this->Surf_City2->getCatAndSubCatList()) {
      			$success = true;
      			$message = 'Ok';
		}else{
      			$success = false;
		...
		$ar = array('user_id'=>$this->input->post('user_id'));
    		if($dataList = $this->Surf_City2->getUserInCart($ar)){
    			...
    			if($dataList = $this->Surf_City2->getInCartShopCheck($array)){
    				...
    				if($dataList = $this->Surf_City2->getItemCheck($arrayi)){
    					//have same shop in cart---OK
    					$success = true;
		...
		$success = false;
    		...
    		if($dataList = $this->Surf_City2->getUserInCart($ar)){
    			...
    			if($dataList = $this->Surf_City2->getInCartShopCheck($array)){
    				...
    				if($idItem = $this->Surf_City2->updateItemToCart($cartItemArray,$idItemCart)){
    					$arrayQut = array('id'=>$idItem);
    					$quantity = $this->Surf_City2->getItemCheck($arrayQut);
    					$id = '';
    				...
    				if($idItem = $this->Surf_City2->addItemToCart($cartItemArray)){
    					$arrayQut = array('id'=>$idItem);
    					$quantity = $this->Surf_City2->getItemCheck($arrayQut);
    					$id = '';
    					foreach ($quantity as $key => $value) {
    			...
    			if($id = $this->Surf_City2->addToCart($cartArray)){
		...
		$success = false;
    		...
    		if($this->input->post('item_id')&&$this->input->post('owner_id')&&$this->input->post('user_id')){
    			...
    			$qut = $this->input->post('quantity');
    				...
    				if($qut>0){
    				$cartItemArray = array('quantity'=>$this->input->post('quantity'),'price'=>$this->input->post('price'));
    				if($idItem = $this->Surf_City2->updateRmItemToCart($cartItemArray,$idItemCart)){
    					$arrayQut = array('id'=>$idItem);
    					$quantity = $this->Surf_City2->getItemCheck($idItemCart);
    					$id = '';
    					foreach ($quantity as $key => $value) {
    				  		$id = $value->quantity;
    					}
    					...
    					$this->Surf_City2->removeItemFromCart($idItemCart,$recart);
    					$success = true;
    					$message = 'Quantity Removed From Cart';
    					$msgCart = 0;
    				}

    			echo json_encode(array('success' => $success , 'message'=> $message , 'params' => '','cat_list' => $idItem,'msg_cart'=>$msgCart));
   ...
   public function getBookingHistory()
    {
      ...
      $this->load->model('Surf_City2');
      $checkProfile = $this->Surf_City2->getBookingHistory($this->input->post());
     ...
     public function changeForgotPass()
    {
      ...
      $this->load->model('Surf_City2');
      $checkProfile = $this->Surf_City2->changeForgotPass($phone,$user_password);
      if ($checkProfile) {
        $success = true;
  ...
  public function bookAppointment()
    ...
    public function getProfileData()
    {
      ...
      if($this->input->post('user_id')){
      $this->load->model('Surf_City2');
      $checkProfile = $this->Surf_City2->getProfileData($this->input->post('user_id'));
    ...
    public function updateUserInfo()
    {
      ...
      if($this->input->post('user_id')){
      $this->load->model('Surf_City2');
      $checkProfile = $this->Surf_City2->updateUserInfo($this->input->post());
