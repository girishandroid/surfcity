                            ...
                            <?php foreach ($getOrderList as  $val) {  $value = $val['orderDetails'];   ?> <tr>
                                ...
                                foreach($cityList as $city):
                                 if($value->shop_city==$city->id) echo $city->city_name;  endforeach; ?><br></td>
                                  ...
                                  <td><button type="button" onclick="changeStatusOrdered(<?php echo $value->id; ?>)" class="btn btn-success btn-block">Confirm Order</button></td>
                                  <td><button type="button" onclick="cancelStatusOrdered(<?php echo $value->id; ?>)" class="btn btn-red btn-block">Cancel Order</button></td>
                                </tr>
...
<!--get List Of Table-->
<script>
//Change Notify Status Start
$(function() {
    $.ajax({
           type: "POST",
           url: "<?php echo site_url(); ?>/welcome/changeOrderNotify",
           data: { 'is_notify' : 1 },
           dataType:"json",
          cache: "false",
   success: function(response)
   {
       var noti = document.getElementById('notification_count_1').innerHTML = '0';
	var noti = document.getElementById('notification_count_2').innerHTML = '0 New';
    },
...
function cancelStatusOrdered(id) {
    if (confirm("Are you really want to Cancel This Order ?") == true) {
    $.ajax({
           type: "POST",
           url: "<?php echo site_url(); ?>/welcome/changeStatusOrdered",
           data: { 'id' : id, 'status' : 2 },
           dataType:"json",
          cache: "false",
   success: function(response)
    ...
    if(response.success)
        {
           ...
           displaySuccessToast("Order status Change Success",response.message);
           window.location.reload();

        }
     else
       {
           displayDangerToast("Order status Change Fail Please Try Again",response.message);
       }
    },
         error: function(jqXHR,textStatus,errorThrown){
         alert(jqXHR.result);
...
function changeStatusOrdered(id) {
  if (confirm("Are you really want to Confirm This Order ?") == true) {
    $.ajax({
           type: "POST",
           url: "<?php echo site_url(); ?>/welcome/changeStatusOrdered",
           data: { 'id' : id, 'status' : 1 },
